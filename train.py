...
from utils.utils import init_comet, log_metrics, setup_logging
...
def load_checkpoint(model: Pix2Pix, config: Config):
    ...
    if not disc_checkpoint.exists():
        raise FileNotFoundError(
            f"Generator checkpoint file not found: {disc_checkpoint}\nPlease check config.yaml"
        )
...
def validate(
    model: Pix2Pix, val_loader: DataLoader, device: torch.device, epoch, experiment
):
    """Validate the model"""
    model.eval()
    total_lossD, total_lossG = 0.0, 0.0
